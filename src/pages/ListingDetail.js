...
function BanknoteIcon({ size = 13 }) {
  return (
    <svg width={size} height={size} viewBox="-22 -14 44 28" style={{ verticalAlign: '-2px' }}>
      <rect x="-20" y="-12" width="40" height="24" rx="4" fill="none" stroke="#0F6E56" strokeWidth="4" />
...
export default function ListingDetail() {
  ...
  useEffect(() => {
    supabase
      .from('listings')
      .select('*, profiles!listings_user_id_fkey(full_name, company, bio, location, avatar_url, deleted)')
      .eq('id', id)
      .single()
