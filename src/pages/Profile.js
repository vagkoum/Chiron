  ...
  return (
    <div className="page-narrow">
            ...
            {l.status === 'sold' ? (
              ...
              <button className="btn btn-outline btn-sm" onClick={() => toggleListing(l.id, l.active)} title={l.active ? 'Hide this listing from Browse' : 'Make this listing visible in Browse again'}>
                {l.active ? 'Pause' : 'Activate'}
                        ...
                        {l.canDelete ? (
              <button className="btn btn-danger btn-sm" onClick={() => deleteListing(l.id)} title="Permanently delete this listing">Delete</button>
            ) : (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        <button className="btn btn-outline btn-sm" onClick={() => navigate('/export-data')} title="Download a copy of your data">
          Export my data
        </button>
